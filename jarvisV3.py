...
def startJarvis():
    ...
    try:
        ...
        print("You said: " + text)
        # message.append(text)
        # put audio throught responce function
        responce = getResponce(text)
        # message.append(responce)
...
def getResponce(input):
    ...
    #     responce = traslation.text

    if input.find("definition") > -1 or input.find("meaning") > -1:
        restOfText = ""

        startIndex = input.find("of") + 2
        restOfText = input[startIndex:]
        meaning = dictionary.meaning(restOfText)
        responce = "The meaning of " + restOfText + " is. " + meaning.get(list(meaning)[0])[0]
        ...
        # fail safe to make sure email is correct
        spelledEmail = ""
        for l in list(startEmail):
            spelledEmail += l + ", "

        engine.say("Is this email correct " + spelledEmail + ", at " + endEmail + ". Respond yes or no")
        engine.runAndWait()
        safeEmail = recordAudio()
        
        if safeEmail.find("yes") == -1:
            engine.say("Please say the correct email")
            # message.append("Who would you like to send an email to")
            engine.runAndWait()
            safeEmailText = recordAudio()
            # extracting email
            startEmail = (safeEmailText[:safeEmailText.find("at")].strip())
            endEmail = (safeEmailText[safeEmailText.find("at") + 2:].strip())
            to_email = str(startEmail + "@" + endEmail)
            to_email = to_email.replace(" ", "")
            print("Email: " + to_email)
        ...
        try:
            ...
            responce = "I cannot find information on " + search + ". Please make your request more specific"
        ...
        responce = getCurrentWeather(input)

    elif input.find("open") > -1:
        app = input[(input.find("open") + 4):].strip()
        app = app.title()
        responce = "Opening " + app
        # os.system('open ' +d+'/%s.app' %app.replace(' ','\ '))
        os.system('open ' +d+'/%s.app' %(app.replace(' ','\ ').replace(' ','')))
        ...
        for w in list(word):
            spelledWord += w + ", "
        responce = word + " is spelled, " + spelledWord

    elif input.find("flip a coin") > -1:
        r = int(random() * 2)
        results = ["heads", "tails"]
        responce = "I flipped " + results[r]
        ...
        r = int(random() * 7)
        ra = int(random() * 7)
        responce = str(r) + " and. " + str(ra)

    elif input.find("lower") > -1 or input.find("whisper") > -1:
        engine.setProperty('volume', 0.1)
        responce = "I will wisper until you say stop whispering"

    elif input.find("stop whispering") > -1:
        engine.setProperty('volume', 1)
        responce = "I will talk at a normal volume now"
        ...
        responce = "Please repeat your command"
